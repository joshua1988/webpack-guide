# NPM-Local-Setting
 ...
 ## 로컬환경 설정이 필요한 이유
 >>인터넷이 불가능한 로컬 환경에서 파일을 가지고와서 npm을 실행하는 방법을 공유합니다.

 ## 로컬 환경 설정을 위한 준비사항
  - node_modules  <b>npm i npm 설치!! (필수)</b>
  - node.exe 파일 다운로드 받기 -> 버전별 LTS 별로 32비트,    64비트 환경을 확인해주세요
  - node_etw_provider.man
  - nodevars.bat 파일 생성
  - npm 파일 생성
  - npm.cmd 파일 생성

 >> 디렉토리 파일을 모두 압축해서 로컬PC로 자료를 이동하시면 됩니다.
 
## node_etw_provider.man
::: tip
 파일 생성  : node_etw_provider.man 
 파일생성 후 아래코드를 복사 후 저장
 최상위 root 경로에 파일을 생성
:::
